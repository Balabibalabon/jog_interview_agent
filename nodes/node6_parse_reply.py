...
- 若使用者說「都不用」、「不跟進」、「不要」、「No」、「none」等否定語意，回傳空清單
- 若使用者指定編號（如「1、3」），回傳對應的 job_id
- 若使用者指定公司或職稱名稱，回傳對應的 job_id
- 若清單只有一個職缺，且使用者回覆帶有正面或同意語意（例如「跟進」、「開始執行」、「好」、「ok」、「要」、「是」、「yes」、「可以」），視為同意跟進該職缺，回傳那個 job_id
- 若清單只有一個職缺，且使用者回覆帶有否定語意（例如「不要」、「不用」、「No」、「算了」、「跳過」），回傳空清單
- 無法判斷時，回傳空清單
